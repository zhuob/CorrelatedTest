...
%	 for two
%	sample $t$-test. In the first part, we conclude theoretically that test statistics correlation 
%and
...
%	\subsubsection{$S$ is a constant}
	...
	\begin{theorem}\label{thm:teststatcor}
		Let $(X_j, Y_j), j = 1, \ldots, m$ be independent random vectors with mean and covariance structures specified in equation (\ref{eq:meanstruct}). If $(\bm a^T\bm X, \bm a^T\bm Y)$ is independent of $(S_X, S_Y)$, then the correlation of $T_X$ and $T_Y$ in equation (\ref{eq:teststat}) can be expressed as 
		\begin{equation}\label{eq:teststatcor}
			\rho_T(m) = \frac{ \rho E(S_X^{-1}S_Y^{-1}) + \frac{\bm a^T\bm \mu_X\cdot \bm a^T\bm 
			\mu_Y}{\sigma_X\sigma_Y\bm a^T\bm a}\cov(S_X^{-1}, S_Y^{-1}) }{\sqrt{\left[E(S_X^{-2}) 
			+ \frac{(\bm a^T\bm \mu_X)^2}{\sigma_X^2\bm a^T\bm 
			a}\var(S_X^{-1})\right]\left[E(S_Y^{-2}) + \frac{(\bm a^T\bm \mu_Y)^2}{\sigma_Y^2\bm 
...
%		$r_\text{statistics}$ and $r_{\text{sample}}$ should reflect the true correlation between $	
%%% 		T_X$ and $T_Y$ and 
...
%	We note that in all of the four cases, the inequality $r_\text{statistics}\leq \rho$ holds.
	...
	\begin{figure}[!th]
		\centering
		\includegraphics[width=4.4cm, height= 6.2cm]{Figures/case1.eps}
		\includegraphics[width=4.4cm, height= 6.2cm]{Figures/case2.eps}
		\includegraphics[width=4.4cm, height= 6.2cm]{Figures/case3.eps}
		\includegraphics[width=4.4cm, height= 6.2cm]{Figures/case4.eps}
		\includegraphics[width=4.4cm, height= 6.2cm]{Figures/case5.eps}
		\includegraphics[width=4.4cm, height= 6.2cm]{Figures/case6.eps}
	\caption{Plots of test statistics correlation against true population 
	correlation. The test statistics are calculated 
	using two sample $t$-test with equal variance, and the theoretical correlation is calculated by 
...
%	  \end{equation}
	...
	\textbf{Proof}: Let's first look at samples $j=1, \ldots, n_1$. Note that 
	 ...
	 \end{equation}
	 is the \textit{maximum likelihood estimator} (MLE) for $\sigma_X^2$. By invariance property of MLE,
