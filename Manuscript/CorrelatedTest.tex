...
%	\subsubsection{$S$ is a constant}
	...
	\begin{theorem}\label{thm:teststatcor}
		Let $(X_j, Y_j), j = 1, \ldots, n_1 + n_2$ be independent random vectors with mean and covariance structures specified in equations (\ref{eq:meanstruct}) and (\ref{eq:covstruct}), and $E(\bm a^T\bm X, \bm a^T\bm Y) = (\Delta_X,\Delta_Y)$. If $(\bm a^T\bm X, \bm a^T\bm Y)$ is independent of $(S_X, S_Y)$, then the correlation of $T_X$ and $T_Y$ in equation (\ref{eq:teststat}) can be expressed as 
		\begin{equation}\label{eq:teststatcor}
			\rho_T = \frac{\frac{\Delta_X\Delta_Y}{\sigma_X\sigma_Y\bm a^T\bm a}\cov(S_X^{-1}, S_Y^{-1}) + \rho E(S_X^{-1}S_Y^{-1})}{\sqrt{\left[E(S_X^{-2}) + \frac{\Delta_X^2}{\sigma_X^2\bm a^T\bm a}\var(S_X^{-1})\right]\left[E(S_Y^{-2}) + \frac{\Delta_Y^2}{\sigma_Y^2\bm a^T\bm a}\var(S_Y^{-1})\right]}}
		\end{equation}
	\end{theorem}
	...
	\begin{aligned}
		...
		\var(\bm a^T\bm X)&  = \sigma_X^2\bm a^T\bm a, \\
		 E(\bm a^T\bm X)^2& =\Delta_X^2 + \sigma_X^2\bm a^T\bm a, \\
		E[\bm (\bm a^T\bm X)(\bm a^T\bm Y)] &=E(\bm a^T\bm X)E(\bm a^T\bm Y) + \cov(\bm a^T\bm X, \bm a^T\bm Y)  	\\& = 	\Delta_X\Delta_Y + \rho \sigma_X\sigma_Y\bm a^T\bm a
	\end{aligned}
	\end{equation}
	...
	\begin{aligned}
		...
		& = \sigma_X^2\bm a^T\bm a E(S_X^{-2}) + \Delta_X^2\var(S_X^{-1})
	\end{aligned}
	\end{equation}
	Similarly, 
	\begin{equation}\label{eq:testdenom2}
	\var(T_Y)= \sigma_Y^2\bm a^T\bm a E(S_Y^{-2}) + \Delta_Y^2\var(S_Y^{-1})
	\end{equation}
	and
	...
	\begin{aligned}
		&\cov(T_X, T_Y) = E\left[\frac{ \bm a^T\bm X }{S_X^{-1}}\cdot\frac{ \bm a^T\bm Y }{S_Y^{-1}}\right] - E\left[\frac{\bm a^T\bm X}{S_X^{-1}}\right]E\left[\frac{\bm a^T\bm Y}{S_Y^{-1}}\right] \\
			 & = E[\bm (\bm a^T\bm X)(\bm a^T\bm Y)]\cdot E[S_X^{-1}S_Y^{-1}]-  \Delta_X\Delta_YE[S_X^{-1}]E[S_Y^{-1}]\\
			 & = (\Delta_X\Delta_Y + \rho \sigma_X\sigma_Y\bm a^T\bm a)E[S_X^{-1}S_Y^{-1}]- \Delta_X\Delta_YE[S_X^{-1}]E[S_Y^{-1}]
	\end{aligned}
	\end{equation}	
	...
	\begin{equation}
	\cor(T_X, T_Y) = \frac{\frac{\Delta_X\Delta_Y}{\sigma_X\sigma_Y\bm a^T\bm a} \rho_s + \rho \frac{E(S_X^{-1}S_Y^{-1})}
	{\sqrt{\var(S_X^{-1})\var(S_Y^{-1})}}	
		}{\sqrt{\left[ \frac{E(S_X^{-2})}{\var(S_X^{-1})} + \frac{\Delta_X^2}{\sigma_X^2\bm a^T\bm a}\right]\left[ \frac{E(S_Y^{-2})}{\var(S_Y^{-1})} + \frac{\Delta_Y^2}{\sigma_Y^2\bm a^T\bm a}\right]}} 
	\end{equation}
	where 
	\begin{equation}
	...
	Under two sample $t$-test with equal variance, we have the following theorem,
