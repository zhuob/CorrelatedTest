...
sample.stat.correlation <- function(lambda, r, n, nreps, delta, method = "Score")
{
  ...
  y.t1 <- correlated.poisson(lambda, r, n/2*nreps)
  # control
  y.t2 <- correlated.poisson(lambda + delta, r, n/2*nreps)
  # here's the idea: since this algorithm is slow in estimating key parameters
  # I'll simulate the desired number of samples at one time. Then I divide them
  # into nreps part
  y <- rbind(y.t1, y.t2)                                           # expression data of two genes
  
  for ( k in 1: nreps)
  {
    y1 <- y[1:2, ((k-1)*n/2 +1):((k-1)*n/2 +n/2)]
    y2 <- y[3:4, ((k-1)*n/2 +1):((k-1)*n/2 +n/2)]
    y.temp <- cbind(y1, y2)
    #
    if (method=="Wald")
    {  u.stat[k, ] <- wald.stat(y.temp)
    }
    else if (method== "LR")
    {
      u.stat[k, ] <- lr.stat(y.temp)
      
    }
    else {u.stat[k, ] <- score.stat(y.temp)}
    
  }                                    # the score test statistics
  
  rho1 <- cor(t(y.t1))[1, 2]; rho2 <- cor(t(y.t2))[1, 2]
  rho.ave <- mean(c(rho1, rho2))                                       # the overall mean as the sample mean
...
ggplot(data = store.corr1, aes(x=true, y=value) ) +
  geom_point(aes(colour=variable, shape=variable), size=1.5) + 
...
###########################################################################
##  NO DE BETWEEN TRT/CONTL, BUT DE BETWEEN GENE 1 AND GENE 2.
...
param.setup <- function(lambda0, lambda2=seq(1, 1000, by =.1), rate=1/10)
{
...
######################################  using Lisa's method to generate correlated Poisson

source("/Users/Bin/Google Drive/Study/Thesis/Correlation/TheoryPart/SimPoissTSPearsonExample.R")

sample.stat.correlation2 <- function(mu1, mu2, rho, n, nreps)
#  mu1 : expression value for trt gene 1 and gene 2 in trt group
#  mu2   
{
...
store.corr <- read.table("/Users/Bin/Google Drive/Study/Thesis/Correlation/Data/Pois5.txt", header = T)
store.corr <- store.corr[complete.cases(store.corr), ]
colnames(store.corr) <- c("sample", "test.stat", "true")             # give them the names
...
y.t1 <- generate.pos(mu1, rho, n/2*nreps)	 # treatment
y.t2 <- generate.pos(mu2, rho, n/2*nreps)	 # treatment
# here's the idea: since this algorithm is slow in estimating key parameters
# I'll simulate the desired number of samples at one time. Then I divide them
# into nreps part
y <- rbind(y.t1, y.t2)                                           # expression data of two genes
rho1 <- cor(t(y.t1))[1, 2]; rho2 <- cor(t(y.t2))[1, 2]
rho.ave <- mean(c(rho1, rho2)) 
rho.ave
